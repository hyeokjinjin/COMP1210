...
public class BuyerProcessor {
   ...
   public void readBuyerFile(String fileName) throws FileNotFoundException {
      ...
      while (buyerFile.hasNextLine()) {
         String readingLine = buyerFile.nextLine();
         Scanner lineReader = new Scanner(readingLine);
         try {
            //Scanner lineReader = new Scanner(buyerFile.nextLine());
            int category = Integer.parseInt(lineReader.next());
            ...
            String exceptionMess = "*** " + e + " in:\n" + readingLine;
            addInvalidRecord(exceptionMess);
         }
   ...
   /**
    * Method that generates a report with the invalid records.
    * @return output Generated report with invalid records.
    */
   public String generateInvalidRecordsReport() {
      String output = "-----------------------\n";
      output += "Invalid Records Report\n";
      output += "-----------------------\n";
      for (String error : stringArray) {
         output += error + "\n\n";
      }
      return output;
   }
